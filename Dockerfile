...
COPY 1.0.0.tar.gz /usr/lib/erlang/releases/1.0.0.tar.gz
RUN cd /usr/lib/erlang && \
	tar zxf releases/1.0.0.tar.gz && \
	erl -noinput \
		-eval "application:start(sasl)" \
		-eval "release_handler:set_unpacked(\"releases/1.0.0.rel\",[])" \
		-s init stop && \
	sed -i 's/unpacked/current/' releases/RELEASES && \
	erl -noinput \
		-eval "application:start(sasl)" \
		-eval "release_handler:make_permanent(\"1.0.0\")" \
		-s init stop
